...
export function prefixStyle(style) {
  ...
  for (let key in browserPrefix) {
    if (baseStyle) {
      let cssPrefixStyle = browserPrefix[key]
        ? browserPrefix[key] + "-" + baseStyle
        : baseStyle;
...
export function nodeIndex(el) {
  return Array.prototype.findIndex.call(
    getElement(el).parentNode.children,
    item => getElement(el).isSameNode(item)
  );
}
